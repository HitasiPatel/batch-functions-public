...
class Task:
    ...
    def __init__(self, settings: Settings = getSettings()) -> None:
        ...
        self.mountConfig = f"--mount source=/{settings.RAW_ZONE_CONTAINER},target=/{settings.RAW_ZONE_CONTAINER},type=bind --mount source=/{settings.EXTRACTED_ZONE_CONTAINER},target=/{settings.EXTRACTED_ZONE_CONTAINER},type=bind"
    ...
    def createTask(
        ...
        taskLevelMountOptions = ""
        if traceparent is not None:
            taskLevelMountOptions = (
                f" -e AZURE_LOG_LEVEL={self.settings.AZURE_LOG_LEVEL} -e ENABLE_LOGS_ON_TRACES"
                f"={self.settings.ENABLE_LOGS_ON_TRACES}"
            )
        ...
        if image is not None:
            task.container_settings = batchModels.TaskContainerSettings(
                image_name=image,
                container_run_options=f"{self.mountConfig}{taskLevelMountOptions}",
            )
